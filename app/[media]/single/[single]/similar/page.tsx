...
export default async function SimilarPage({ params, searchParams }: PageProps) {
  ...
  const data =
    media === "movies"
      ? await getSimilarMovies(single, currentPage)
      : await getSimilarShows(single, currentPage);
  ...
  return (
    <Container>
      <h1 className="text-2xl font-semibold mb-4">Similar</h1>
      ...
      <div className="flex justify-center gap-4 max-sm:mt-4">
        {currentPage > 1 && (
          <Link
            href={`/${media}/single/${single}/similar?page=${currentPage - 1}`}
